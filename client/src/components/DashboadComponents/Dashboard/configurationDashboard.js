...
export const buildStageDonutChart = (stages) => {
    const values = Object.values(stages);
    const hasData = values.some((count) => count > 0);
    if (!hasData)
        return null;
    const labels = Object.keys(stages).map((key) => key.replace("stage", "Stage "));
    const series = values;
    const options = {
        chart: { type: "donut" },
        labels,
...
export const buildStackedFilesChart = (fileUploadedByMonth = []) => {
...
// --- PROJECT STATUS PIE ---
export const buildStatusPieChart = (data) => {
...
export const buildProjectsCreatedPerDayChart = (data) => {
    if (!Array.isArray(data) || data.length === 0)
        return null;
    data.sort((a, b) => a.day - b.day);
    const labels = data.map((item) => `${item.day}`);
    const seriesData = data.map((item) => item.count);
    if (seriesData.every((v) => v === 0))
        return null;
    const options = {
        chart: { type: "line" },
        xaxis: {
...
export const buildProviderLineChart = (data) => {
    if (!Array.isArray(data) || data.length === 0)
        return null;
    const seriesData = data.map((item) => item.count);
    if (seriesData.every((v) => v === 0))
    ...
    const labels = data.map((item) => `${item.year}-${String(item.month).padStart(2, "0")}-${String(item.day).padStart(2, "0")}`);
    const options = {
        chart: { type: "line" },
        xaxis: {
            categories: labels,
